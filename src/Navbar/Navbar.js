...
const Navbar = () => {
  ...
  const [placeholder, setPlaceholder] = useState('Search "Paneer"');
  

  const totalproducts = useSelector((s) => s.y.product);
  const totalprice = useSelector((s) => s.y.totalprice.toFixed(0));

  const productMatch = (e) => {
    dispatch({ type: actions.SEARCH_PRODUCT, payload: e.target.value });
  };

  useEffect(() => {
    const timer = setInterval(() => {
      if (placeholder === 'Search "Paneer"') {
        setPlaceholder('Search "meat"');
      } else if (placeholder === 'Search "meat"') {
        setPlaceholder('Search "oil"');
      } else if (placeholder === 'Search "oil"') {
        setPlaceholder('Search "vegetables"');
      } else {
        setPlaceholder('Search "Paneer"');
      }
    }, 1000);
        ...
        <div className="searc">
          <div className="searchh">
            <input
              ...
              onChange={productMatch}
            />
          </div>
        </div>
        <div className="buttonright">
          <button className="login btn btn-light  px-5 ">Login</button>

          {totalprice === "0" || totalprice === "-0" ? (
            <div
              onClick={sidebartoggle}
              className=" login btnnn btn btn-success  px-5   "
            >
